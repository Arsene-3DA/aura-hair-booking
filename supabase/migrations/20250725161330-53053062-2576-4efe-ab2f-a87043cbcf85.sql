...
-- Drop the materialized view that depends on the status column
DROP MATERIALIZED VIEW IF EXISTS stylists_with_rating;
...
ALTER TABLE public.bookings 
ADD COLUMN IF NOT EXISTS new_status booking_status DEFAULT 'pending';

-- Copy data to the new column
UPDATE public.bookings 
SET new_status = status::booking_status;

-- Drop the old status column and rename the new one
ALTER TABLE public.bookings DROP COLUMN status;
ALTER TABLE public.bookings RENAME COLUMN new_status TO status;

-- Make required columns NOT NULL where possible
UPDATE public.bookings SET scheduled_at = now() WHERE scheduled_at IS NULL;
ALTER TABLE public.bookings 
