-- Fix the data type mismatch in reservation functions
-- Drop existing functions
DROP FUNCTION IF EXISTS get_admin_reservations();
DROP FUNCTION IF EXISTS get_stylist_reservations();

-- Recreate admin reservations function with correct types
...
BEGIN
  ...
  LEFT JOIN auth.users cu ON r.client_id = cu.id
  LEFT JOIN profiles sp ON r.stylist_id = sp.user_id
  LEFT JOIN auth.users su ON r.stylist_id = su.id
  LEFT JOIN services s ON r.service_id = s.id
  ORDER BY r.created_at DESC;
END;
$$;

-- Recreate stylist reservations function with correct types
