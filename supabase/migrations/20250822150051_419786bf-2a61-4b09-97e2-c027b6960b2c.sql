-- Mettre à jour la fonction pour inclure les créneaux jusqu'à 21h30
CREATE OR REPLACE FUNCTION public.get_public_professional_availability(professional_auth_id uuid, check_date text)
RETURNS TABLE(time_slot text, is_available boolean, booking_duration integer)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Générer les créneaux avec une plage étendue pour inclure 21h30
  RETURN QUERY
  WITH time_slots AS (
    SELECT 
    ...
    CROSS JOIN (SELECT check_date::date as target_date) AS date_info
    -- Combiner date et heure pour créer un timestamp complet
    SELECT 
      to_char(slot_time_raw, 'HH24:MI') as slot_time_str,
      (date_info.target_date + slot_time_raw) as slot_time
  ),
  ...
  SELECT 
    ...
    CASE 
      ...
      -- 6. Vérifier si dans les heures d'ouverture du salon
      WHEN ts.slot_time::time >= open_time AND ts.slot_time::time <= close_time THEN true
      
      -- 7. Par défaut: indisponible si en dehors des heures
      ELSE false
    END as is_available,
