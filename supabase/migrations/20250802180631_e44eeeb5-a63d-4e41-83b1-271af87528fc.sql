-- Fix column references in reservation functions
-- Drop existing functions
DROP FUNCTION IF EXISTS get_admin_reservations();
DROP FUNCTION IF EXISTS get_stylist_reservations();

-- Recreate admin reservations function with only existing columns
...
BEGIN
  ...
  SELECT 
    ...
    NULL::TEXT as client_phone,  -- No phone column in profiles
    sp.full_name::TEXT as stylist_name,
    sp.avatar_url::TEXT as stylist_avatar,
    su.email::TEXT as stylist_email,
    NULL::TEXT as stylist_phone,  -- No phone column in profiles
    NULL::TEXT[] as stylist_specialties,  -- No specialties column in profiles
    NULL::TEXT as stylist_location,  -- No location column in profiles
...
-- Recreate stylist reservations function with only existing columns
