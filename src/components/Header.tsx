  ...
  return <header className="bg-black border-b border-[#FFD700]/20 sticky top-0 z-50">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Login Button */}
          <Button onClick={handleLogin} className="hidden md:flex bg-[#FFD700] text-black hover:bg-[#FFD700]/90 hover:shadow-lg hover:shadow-[#FFD700]/20 transition-all duration-300 border-0 rounded-full px-6">
            Se connecter
          </Button>

          {/* Mobile Menu Button */}
          <button className="md:hidden p-2 text-[#FFD700]" onClick={() => setIsMenuOpen(!isMenuOpen)}>
            {isMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
          </button>
        </div>

        {/* Mobile Navigation */}
        {isMenuOpen && <div className="md:hidden py-4 border-t border-[#FFD700]/20 bg-[#1a1a1a]">
