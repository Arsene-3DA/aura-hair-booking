...
const StylistProfilePage = () => {
  ...
  const { hairdresser: expert, loading: isLoading, error } = useSecureHairdresserData(stylistId);

  // Récupérer les services du professionnel avec mises à jour temps réel
  const { services, loading: servicesLoading } = useProfessionalServices(stylistId, true);
  
  // Récupérer le portfolio avec mises à jour temps réel
  const { portfolio } = usePortfolioManagement(stylistId);
